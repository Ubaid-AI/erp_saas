...
def validate_users(self, count_administrator_user, count_website_users, allowed_users):
    ...
    if allowed_users != 0 and active_users >= allowed_users:
        if not frappe.get_all('User', filters={'name': self.name}):
            frappe.throw('Only {} active {} users allowed and you have {} active users. Please disable users or to increase the limit please contact <a href="https://intraerp.com/" target="_blank">contact sales</a>.'. format(allowed_users, is_desk, active_users))
...
def validate_files_space_limit():
    ...
    if allowed_space != 0 and total_size > allowed_space:
        msg = '''
        <div>You have exceeded your files space limit. Delete some files from file manager or to increase the limit please <a href="https://intraerp.com/" target="_blank">contact sales</a>.</div>
...
def validate_db_space_limit():
    ...
    if allowed_db_space != 0 and used_db_space > allowed_db_space:
        msg = '''
        <div>You have exceeded your Database Size limit. Please <a href="https://intraerp.com/" target="_blank">contact sales</a>. to upgrade your package</div>
...
def company_limit(self, method):
    ...
    if allowed_companies != 0 and total_company >= allowed_companies:
        if not frappe.get_all('Company', {'name': self.name}):
            frappe.throw(_("Only {} company(s) allowed and you have {} company(s).Please remove other company or to increase the limit please <a href=""https://intraerp.com/"" target=""_blank"">contact sales</a>.").format(quota.get('company'), total_company))
...
def document_limit(doc, event):
    ...
    if (limit_dict.get(doc.doctype)):
        ...
        if usage >= limit.limit:
            msg = _(f"Your have reached your {doc.doctype} {limit.period} limit of {limit.limit} and hench cannot create new document. Please <a href=""https://intraerp.com/"" target=""_blank"">contact sales</a>.")
